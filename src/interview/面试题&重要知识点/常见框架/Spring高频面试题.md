...
![](./img/FMmR39Y-WuBThlQy/20220709213529-028790.png)
...
1、[**@Configuration **](/Configuration )** + **[**@Bean **](/Bean )** **

@Configuration用来声明一个配置类，然后使用 [@Bean ](/Bean ) 注解，用于声明一个bean，将其加入到Spring容器中。 
...
特定注解包括@Controller、@Service、@Repository、[@Component ](/Component ) 
...
如下图代码，使用[@Configuration ](/Configuration ) + @Bean的方式将 PersonFactoryBean 加入到容器中，这里没有向容器中直接注入 Person，而是注入 PersonFactoryBean，然后从容器中拿Person这个类型的bean。 
...
Autowire是spring的注解。默认情况下@Autowired是按类型匹配的(byType)。如果需要按名称(byName)匹配的话，可以使用@Qualifier注解与@Autowired结合。[@Autowired ](/Autowired ) 可以传递一个 `required=false`的属性，false指明当userDao实例存在就注入不存就忽略，如果为true，就必须注入，若userDao实例不存在，就抛出异常。
...
都是使用注解定义 Bean。[@Bean ](/Bean ) 是使用 Java 代码装配 Bean，[@Component ](/Component ) 是自动装配 Bean。 

[@Component ](/Component ) 注解用在类上，表明一个类会作为组件类，并告知Spring要为这个类创建bean，每个类对应一个 Bean。 

[@Bean ](/Bean ) 注解用在方法上，表示这个方法会返回一个 Bean。[@Bean ](/Bean ) 需要在配置类中使用，即类上需要加上@Configuration注解。 
...
[@Bean ](/Bean ) 注解更加灵活。当需要将第三方类装配到 Spring 容器中，因为没办法源代码上添加@Component注解，只能使用[@Bean ](/Bean ) 注解的方式，当然也可以使用 xml 的方式。 

<a name="34b88e40"></a>
## @Component、@Controller、@Repositor和[@Service ](/Service ) 的区别？ 
...
1.首先在启动类上添加 [@EnableAsync ](/EnableAsync ) 注解。 
...
![](./img/FMmR39Y-WuBThlQy/20220628224208-877287.png)
...
![](./img/FMmR39Y-WuBThlQy/20220612101342-690167.png)


<!--page footer-->
- 原文: <https://www.yuque.com/dabin-1eu6s/plc2v4/usygrk7ngarn550b>